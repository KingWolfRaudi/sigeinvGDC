            ...
            <div class="row g-3 justify-content-between align-items-center">
                <div class="col-md-5">
                    <div class="input-group shadow-sm">
                        ...
                        <input type="text" wire:model.live.debounce.300ms="search" class="form-control border-start-0 ps-0" placeholder="Buscar departamento...">
                    </div>
                </div>
                ...
                <div class="col-md-4 text-end d-flex gap-2 justify-content-end">
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu shadow border-0">
                            <li><a class="dropdown-item py-2" href="{{ route('reportes.catalogo.excel', ['tipo' => 'departamentos', 'search' => $search, 'estado' => $filtro_estado]) }}"><i class="bi bi-filter me-2 text-success"></i> Vista Actual</a></li>
                            <li><a class="dropdown-item py-2" href="{{ route('reportes.catalogo.excel', ['tipo' => 'departamentos']) }}"><i class="bi bi-list-check me-2 text-primary"></i> Todo el Catálogo</a></li>
                        </ul>
                    </div>
                    @endcan
                            ...
                            <tr>
                                ...
                                <td class="text-end">
                                    @can('cambiar-estatus-departamentos')
                                        <button wire:click="toggleActivo({{ $departamento->id }})" class="btn btn-sm {{ $departamento->activo ? 'btn-success' : 'btn-secondary' }} text-white" title="Alternar Estado">
                                            <i class="bi {{ $departamento->activo ? 'bi-toggle-on' : 'bi-toggle-off' }}"></i>
                                        </button>
                                    @endcan

                                    @can('ver-departamentos')
                                        <button wire:click="ver({{ $departamento->id }})" class="btn btn-sm btn-info text-white" title="Ver Detalles">
                                            <i class="bi bi-eye"></i>
                                        </button>
                                    @endcan
                                    
                                    @can('editar-departamentos')
                                        <button wire:click="editar({{ $departamento->id }})" class="btn btn-sm btn-primary" title="Editar">
                                            <i class="bi bi-pencil-square"></i>
                                        </button>
                                    @endcan
                                    
                                    @can('eliminar-departamentos')
                                        <button wire:click="eliminar({{ $departamento->id }})" wire:confirm="¿Deseas eliminar este departamento?" class="btn btn-sm btn-danger" title="Eliminar">
                ...
                <form wire:submit.prevent="guardar">
                    <div class="modal-body" style="max-height: 65vh; overflow-y: auto;">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('nombre') is-invalid @enderror" wire:model="nombre" placeholder="Ej: Recursos Humanos, TI">
                ...
                <div class="modal-body" style="max-height: 65vh; overflow-y: auto;">
                    @if($departamento_detalle)
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item"><strong>ID:</strong> {{ $departamento_detalle->id }}</li>
                            ...
                            <li class="list-group-item"><strong>Creado el:</strong> {{ $departamento_detalle->created_at->format('d/m/Y H:i') }}</li>
                        </ul>
                    @endif
                </div>
