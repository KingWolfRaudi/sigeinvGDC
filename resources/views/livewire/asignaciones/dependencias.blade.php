<div>
        ...
        <div class="row mb-4 align-items-center">
            <div class="col-12 d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <h2 class="fw-bold mb-0 text-body">Catálogo de Dependencias</h2>
                        <p class="text-muted mb-0">Administración de subdivisiones, direcciones y coordinaciones.</p>
                    </div>
                </div>
                <div class="text-end">
                    <div class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 px-3 py-2 rounded-pill shadow-sm">
                        <i class="bi bi-collection me-1"></i> Total Dependencias: <span class="fw-bold fs-6 ms-1">{{ $dependencias->total() }}</span>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="col-md-5">
                    <div class="input-group shadow-sm">
                        ...
                        <input type="text" wire:model.live.debounce.300ms="search"
                            class="form-control border-start-0 ps-0" placeholder="Buscar dependencia o departamento...">
                    </div>
                </div>
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu shadow border-0">
                                <li><a class="dropdown-item py-2"
                                        href="{{ route('reportes.catalogo.excel', ['tipo' => 'dependencias', 'search' => $search, 'estado' => $filtro_estado]) }}"><i
                                            class="bi bi-filter me-2 text-success"></i> Vista Actual</a></li>
                                <li><a class="dropdown-item py-2"
                                        href="{{ route('reportes.catalogo.excel', ['tipo' => 'dependencias']) }}"><i
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-light">
                        <tr>
                            <th wire:click="sortBy('nombre')" style="cursor: pointer;">
                                Nombre Dependencia @if($sortField === 'nombre') <i class="bi bi-sort-alpha-{{ $sortAsc ? 'down' : 'up' }} ms-1"></i> @endif
                            </th>
                    ...
                    <tbody>
                        @forelse($dependencias as $dependencia)
                            <tr>
                                <td class="fw-bold">{{ $dependencia->nombre }}</td>
                                <td>{{ $dependencia->departamento->nombre ?? 'N/A' }}</td>
                                @can('ver-estado-departamentos')
                                    <td>
                                ...
                                <td class="text-end">
                                    @can('cambiar-estatus-departamentos')
                                        <button wire:click="toggleActivo({{ $dependencia->id }})"
                                            class="btn btn-sm {{ $dependencia->activo ? 'btn-success' : 'btn-secondary' }} text-white"
                                            title="Alternar Estado">
                                            <i class="bi {{ $dependencia->activo ? 'bi-toggle-on' : 'bi-toggle-off' }}"></i>
                                        </button>
                                    @endcan

                                    @can('ver-departamentos')
                                        <button wire:click="ver({{ $dependencia->id }})" class="btn btn-sm btn-info text-white"
                                            title="Ver Detalles">
                                            <i class="bi bi-eye"></i>
                                        </button>
                                    @endcan
                                    ...
                                    @can('editar-departamentos')
                                        <button wire:click="editar({{ $dependencia->id }})" class="btn btn-sm btn-primary"
                                            title="Editar">
                                    ...
                                    @can('eliminar-departamentos')
                                        <button wire:click="eliminar({{ $dependencia->id }})"
                                            wire:confirm="¿Deseas eliminar esta dependencia?" class="btn btn-sm btn-danger"
                                            title="Eliminar">
    ...
    <!-- Modal Formulario -->
    <div wire:ignore.self class="modal fade" id="modalDependencia" tabindex="-1" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <form wire:submit.prevent="guardar">
                    <div class="modal-body">
                        <div class="mb-3">
                            <label class="form-label">Departamento Padre <span class="text-danger">*</span></label>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('nombre') is-invalid @enderror"
                                wire:model="nombre" placeholder="Ej: Dirección General, División de Redes">
    ...
    <!-- Modal Detalle -->
    <div wire:ignore.self class="modal fade" id="modalDetalleDependencia" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    @if($dependencia_detalle)
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item"><strong>Nombre:</strong> {{ $dependencia_detalle->nombre }}</li>
                            <li class="list-group-item"><strong>Departamento:</strong> {{ $dependencia_detalle->departamento->nombre ?? 'N/A' }}</li>
                            <li class="list-group-item">
                                <strong>Estado:</strong>
                                @if($dependencia_detalle->activo)
                                    <span class="badge bg-success">Activo</span>
                                @else
                                    <span class="badge bg-danger">Inactivo</span>
                                @endif
                            </li>
                            <li class="list-group-item"><strong>Creado el:</strong> {{ $dependencia_detalle->created_at->format('d/m/Y H:i') }}</li>
                        </ul>
                    @endif
                </div>
                <div class="modal-footer d-flex justify-content-between">
                    @if($dependencia_detalle)
                        <a href="{{ route('asociaciones', ['tipo' => 'dependencia', 'id' => $dependencia_detalle->id]) }}"
